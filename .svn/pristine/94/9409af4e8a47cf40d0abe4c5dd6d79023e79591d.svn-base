...
{
    public class QXCStructure: SPImageStructure
    {
        public QXCStructure(String lNameStr)
       {
           ...
           this.HIGHLY_EFFECTIVE_CONTENT = 36 * SPImageGlobal.BB_HIGH;

           for (int i = 0; i < this.SLIP_EVENT_NUMBER; i++)
           {
               this.DATA_BLOCK_LIST.Add(new QXC_DataBlock(SPImageGlobal.LEFT_SMALL_BB_X,
                               SPImageGlobal.START_POINT_Y + (i*7+1) * SPImageGlobal.BB_HIGH));
           }
            //倍数块
        ...
        public override List<Point> getDrawPoints(lottery_ticket lt)
        {
            ...
            if (lt.play_type.Equals(QXCPlayType.ZHX_FS.ToString()))//复式
            {
                points.Add(new Point(SPImageGlobal.LEFT_SMALL_BB_X + 11 * SPImageGlobal.S2S_WIDTH,
