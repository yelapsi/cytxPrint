...
{
    public class QXC_DataBlock: ImageDataBlock
    {
       public static int DATA_BLOCK_HIGH = 7 * SPImageGlobal.BB_HIGH;

       public QXC_DataBlock(int startx, int starty)
       {
       ...
       public override void getPointArrayByData(List<Point> pointlist, String data)
        {
            ...
            for (int i = 0; i < dz.Length; i++)
            {
                ...
                foreach (char item in results)
                {
                    ...
                    pointlist.Add(
                        new Point(SPImageGlobal.LEFT_SMALL_BB_X + (x+1) * SPImageGlobal.S2S_WIDTH,
