...
{
    public class SCJQ_DataBlock: ImageDataBlock
    {
       public static int DATA_BLOCK_HIGH = 10 * SPImageGlobal.BB_HIGH;

       public SCJQ_DataBlock(int startx, int starty)
       {
       ...
       public override void getPointArrayByData(List<Point> pointlist, String data)
        {
            ...
            for (int i = 0; i < dz.Length; i++)
            {
                ...
                {
                    pointlist.Add(
                        new Point(this.startPoint.X + (item == '0' ? 0 : (item == '1' ? 1 : (item == '2' ? 2 : 3))) * SPImageGlobal.S2S_WIDTH,
